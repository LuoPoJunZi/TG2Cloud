...
再通过 rclone 和 CloudDrive2 或 OpenList WebDAV 写入自己挂载的 115 网盘。
...
- [OpenList 与 115 配置](#openlist-与-115-配置)
...
- TG115 只验证所选 WebDAV 目的端已收到同样大小的文件，不接入 115 官方完成接口。
...
CloudDrive2 当前正式版为 `v1.6.2`；OpenList 产品线首版正在开发和实机验收，尚未发布正式
Release。两版共用 TG115 任务核心，但使用独立安装目录、容器、网络和备份，不会互相覆盖。
...
```text
dist/TG115-CloudDrive2-Deployer.exe
dist/TG115-OpenList-Deployer.exe
```
...
[OpenList 与 115 配置](#openlist-与-115-配置)中的五步流程操作。
...
OpenList 版使用独立的 `/opt/tg115-openlist` 安装目录、`tg115-openlist` 容器和
`tg115-openlist-net` 网络，可与 CloudDrive2 版共存；但不要让两个实例同时长期使用同一个
Telegram Bot Token。
...
3. 点击“一键部署基础环境”。此时只要求 OpenList、Bot 和内部网络健康；尚未添加 115 或尚未
...
5. 登录 OpenList，由本人在后台添加并授权 `115 Open` 存储。115 Cookie、Token 和登录信息
   只交给你自己的 OpenList，TG115 部署器不会读取或收集。
6. 按“配置 WebDAV”页的同一组值创建专用普通用户：推荐用户名 `tg115`、部署器本地生成的
   28 位随机密码、建议基本路径 `/115/Telegram`。用户名、密码和路径均可直接复制；只有点击
...
8. 回到部署器点击“WebDAV 验收”。只有认证、目录访问、写入、大小检查、改名、复验和删除全部
   完成，并出现 `TG115_DESTINATION=OK`，才表示 TG115 到 OpenList WebDAV 的链路可用。

关闭部署器后，本地生成的密码不会保存。重新打开用于已有实例升级时，可勾选“保留 VPS 当前
WebDAV 配置”：旧凭据只在 VPS 内合并，不会被读取回 Windows 或输出到日志；如果 VPS 上没有
可复用配置，部署会失败关闭。需要更换凭据时不要勾选，并确保 OpenList 中填写的值与界面一致。
管理员密码恢复属于有影响的高级操作，必须明确确认后使用 VPS 上的官方管理命令。

右侧“OpenList 日常管理”区域提供运行状态、最近脱敏日志、分别重启 Bot／OpenList、创建安全
备份和管理员密码恢复。安全备份包含程序配置、TG115 SQLite 一致性快照与 OpenList 状态；为
避免复制到不一致的 OpenList 数据，操作期间会短暂停止两个服务，完成后自动启动并检查健康。
管理员密码恢复需要两次确认，新密码只写回当前窗口的遮罩输入框，不进入普通日志。

## 真实 WebDAV 验收

CloudDrive2 或 OpenList 与 115 配置完成后，点击“WebDAV 验收”。该操作会从 Bot 容器内使用
实际运行配置：
...
`%APPDATA%\TG115-Deployer\known_hosts` 中其他服务器的记录。不要把错误窗口中的真实 IP、
主机公钥或登录资料提交到 Issue。
...
- OpenList 管理端口同样只绑定 `127.0.0.1:5244`；115 Open 授权由用户直接在自己的后台完成，
  部署器没有凭据遥测或第三方回传功能。
