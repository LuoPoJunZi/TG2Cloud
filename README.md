...
>
> OpenList 已在真实 VPS、OpenList 与 115 Open 环境通过部署和 WebDAV 验收。CloudDrive2
> 继续使用已经在真实 VPS 验收通过的稳定流程；两个正式 EXE 均由 GitHub Actions 从同一个
> `v1.0.2` 标签源码构建。
...
TG2Cloud v1.0.2 的 Windows EXE 当前未提供商业代码签名，首次运行时 Windows SmartScreen
...
回退逻辑，TG2Cloud v1.0.2 不提供正式自动 Restore。
...
TG2Cloud 备份目录，也不清理 TG115 备份。TG2Cloud v1.0.2 不提供正式自动 Restore 或 Uninstall。
