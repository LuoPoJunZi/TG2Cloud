# 从 TG115 迁移到 TG2Cloud v1.0.0

TG2Cloud v1.0.0 默认按全新安装处理，**不提供自动原地迁移**。旧 TG115 目录或已停容器
...
应先制定和验证逐项迁移及回滚步骤，再由操作者明确执行；这不属于 v1.0.0 自动部署流程。
...
v1.0.0 不新增 restore 或 uninstall；请保留旧项目和独立备份，以便人工恢复。
